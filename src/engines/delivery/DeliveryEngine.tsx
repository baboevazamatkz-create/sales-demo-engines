...
type Screen = 'menu' | 'item' | 'cart' | 'checkout' | 'tracking';

export function DeliveryEngine({ config }: { config: DeliveryConfig }) {
  ...
  const [selectedItem, setSelectedItem] = useState<DeliveryMenuItem | null>(null);

  const cart = useCart<DeliveryMenuItem>();
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {screen === 'checkout' && (
        <CheckoutScreen config={config} total={cartTotal} onBack={() => setScreen('cart')} onConfirm={() => setScreen('tracking')} />
      )}
