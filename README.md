...
|------------------|-----------------|---------|
| `cherish`        | cafe-restaurant | Cherish Bar & Kitchen (Самал-2) — premium-пресет, **журнальная вёрстка**: печатное меню с отточием, разделы полосами, конструктор стейка, бронь трёх банкетных залов |
| `pomo-bistro`    | cafe-restaurant | POMO bistro (Самал-2) — premium-пресет в кремово-бордовой палитре: всё меню с ценами, конструктор кофе, детское меню, бронь террасы |
| `cafe-main`      | cafe-restaurant | Café Main (Самал-2) — тот же premium-пресет в светлой кремово-зелёной палитре: завтраки, комбо, конструктор завтрака, бронь веранды |
| `altay-doner`    | cafe-restaurant | ALTAY DONER (Самал-2) — premium-пресет в чёрно-красно-золотой палитре: сторис, конструктор донера, карта гостя, бронь террасы |
| `bereket`        | cafe-restaurant | Bereket — premium-пресет в бордово-золотой палитре |
| `polyn-cafe`     | cafe-restaurant | Кофейня "Полынь" — светлый classic-пресет: доставка/самовывоз/заказ за столик |
...
Демо под реального клиента отличаются друг от друга не только цветом: в
конфиге задаются пресет оболочки (`style.preset` — `premium` / `classic`), вёрстка
premium-оболочки (`style.layout` — `cards` с карточками и плитками либо
`editorial` с печатным меню, серифной типографикой и плавающей
навигацией), светлая или тёмная палитра (`theme.mood`), фоновая геометрия
(`style.pattern` — `turkish` / `mountains` / `checker` / `diamonds` / `petals` / `grid`) и набор модулей
(`modules`), поэтому два заведения одной ниши выглядят и работают
по-разному.
