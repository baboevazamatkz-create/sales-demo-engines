...
|------------------|-----------------|---------|
| `altay-doner`    | cafe-restaurant | ALTAY DONER (Самал-2) — тёмный премиум-пресет: сторис, конструктор донера, карта гостя, бронь террасы |
| `bereket`        | cafe-restaurant | Bereket — тот же премиум-пресет в бордово-золотой палитре |
...
конфиге задаются пресет оболочки (`style.preset`), фоновая геометрия
(`style.pattern` — `turkish` / `mountains` / `grid`) и набор модулей
